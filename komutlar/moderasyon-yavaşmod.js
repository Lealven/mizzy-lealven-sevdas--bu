...
const limit = args[0] ? args[0] : 0;
  if(!limit) {
              var embed = new emran.MessageEmbed()
                .setDescription(`Doğru kullanım: \`${prefix}yavaşmod [0/100]\``)
                .setColor('RANDOM')
                .setTimestamp()
...
if (limit > 100) {
    return message.channel.send(new emran.MessageEmbed().setDescription("**|** Süre Limiti Maksimum `100` Saniye Ayarlanabilir.").setColor('RED'));
}  
