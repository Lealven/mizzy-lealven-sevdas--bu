...
exports.run = async (client, message, args) => {
      ...
      if (!message.member.hasPermission("MANAGE_ROLES")) return message.channel.send(new codeworld.MessageEmbed().setDescription('Bu komudu kullanmak için **Rolleri Yönet** yetkisine sahip olmalısın.').setColor(10038562));
...
await (member.roles.add(role.id))
 message.channel.send(new codeworld.MessageEmbed().setDescription(`${member} İsimli Üyeye \`${role.name}\` İsimli Yetki Başarıyla Verildi !`)  .setFooter('Bu komutu kullanan yetkili ' + message.author.tag, message.author.avatarURL).setColor('#D2EE07'));
